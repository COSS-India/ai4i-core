...
class TritonClient:
    ...
    def get_language_detection_io_for_triton(
        ...
        """Prepare inputs and outputs for language detection inference"""
        try:
            ...
            logger.error(f"Failed to prepare language detection I/O: {e}")
            raise TritonInferenceError(f"Failed to prepare language detection I/O: {e}")
    
    def _get_string_tensor_2d(self, string_values: List[List[str]], tensor_name: str) -> InferInput:
        """Create 2D string tensor for Triton input (for IndicLID)"""
        try:
            # Create 2D array: [[text1], [text2], ...] → shape [batch_size, 1]
            np_array = np.array(string_values, dtype=object)
            ...
            logger.error(f"Failed to create 2D string tensor {tensor_name}: {e}")
            raise TritonInferenceError(f"Failed to create 2D string tensor: {e}")
