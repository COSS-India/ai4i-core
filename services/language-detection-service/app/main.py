...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["language-detection"]
    ...
    if _TELEMETRY:
        try:
            if setup_tracing("language-detection-service"):
    ...
    try:
        ...
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/language-detection"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/language-detection"]
    ...
    async def root():
        return {"service": "language-detection-service", "version": "1.0.0", "status": "running"}
