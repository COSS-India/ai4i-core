"""
SQLAlchemy ORM models for Language Detection database tables.
"""
...
class LanguageDetectionRequestDB(Base):
    ...
    def __repr__(self) -> str:
        return f"<LanguageDetectionRequestDB(id={self.id}, model_id={self.model_id}, status={self.status})>"
...
class LanguageDetectionResultDB(Base):
    ...
    request_id = Column(
        UUID(as_uuid=True),
        ForeignKey("language_detection_requests.id", ondelete="CASCADE"),
        nullable=False,
    )
    ...
    def __repr__(self) -> str:
        return f"<LanguageDetectionResultDB(id={self.id}, request_id={self.request_id})>"
