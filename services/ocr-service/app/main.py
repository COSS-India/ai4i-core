...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["ocr"]
    ...
    try:
        ...
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/ocr"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/ocr"]
    ...
    async def root():
        return {"service": "ocr-service", "version": "1.0.0", "status": "running"}
