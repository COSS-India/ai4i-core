...
Ported from services/platform-core-service/app/services/
notification_alert_email_templates.py (PR #1557, "Reference Email Template"
spec) — that module was never wired to any real trigger there (grep finds no
caller besides its own tests); this consumer is the actual delivery path, so
the rendering logic belongs here, not in platform-core-service. Only the
