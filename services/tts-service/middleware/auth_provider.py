...
def determine_service_and_action(request: Request) -> Tuple[str, str]:
    """TTS-specific service/action resolution.

    Extracts service name from URL path (e.g. ``/api/v1/tts/...`` -> ``tts``).
    Falls back to ``"tts"`` when no known service slug is found.
    """
