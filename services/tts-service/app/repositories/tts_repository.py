...
from app.models.tts import TTSRequestDB, TTSResultDB
...
class TTSRepository:
    ...
    async def create_request(
        ...
        try:
            ...
            logger.info("Created TTS request %s for model %s", request.id, model_id)
            return request
        except Exception as e:
            await self.db.rollback()
            logger.error("Failed to create TTS request: %s", e)
    ...
    async def update_request_status(
        ...
        try:
            ...
            if not request:
                logger.warning("TTS request %s not found", request_id)
                return None
            ...
            await self.db.commit()
            await self.db.refresh(request)
            logger.info("Updated TTS request %s status to %s", request_id, status)
            return request
        except Exception as e:
            await self.db.rollback()
            logger.error("Failed to update TTS request %s: %s", request_id, e)
    ...
    async def create_result(
        ...
        try:
            ...
            logger.info("Created TTS result %s for request %s", result.id, request_id)
            return result
        except Exception as e:
            await self.db.rollback()
            logger.error("Failed to create TTS result for request %s: %s", request_id, e)
    ...
    async def get_request_by_id(self, request_id: UUID) -> Optional[TTSRequestDB]:
        ...
        try:
            ...
            logger.error("Failed to get TTS request %s: %s", request_id, e)
    ...
    async def get_requests_by_user(
        ...
        try:
            ...
            logger.error("Failed to get TTS requests for user %s: %s", user_id, e)
    ...
    async def get_requests_by_status(
        ...
        try:
            ...
            logger.error("Failed to get TTS requests with status %s: %s", status, e)
            raise DatabaseError(f"Failed to get TTS requests: {e}")
