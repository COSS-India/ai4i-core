...
class NerService:
    ...
    def __init__(self, repository: NERRepository, triton_client: NERTritonClient, model_name: str):
        """
        Initialize NER service.

        Args:
            repository: NER repository for database operations
    ...
    async def run_inference(
        ...
        with tracer.start_as_current_span("ner.process_batch") as span:
            try:
                ...
                total_text_length = sum(len(text_input.source or "") for text_input in request.input)
                ...
                logger.info(f"Created NER request {db_request.id} for {len(request.input)} text input(s)")
                ...
                for prediction in predictions:
                    # Convert NerPrediction to JSON-serializable format for entities
                    entities_data = {
                        "nerPrediction": [
                            {
                                "token": token.token,
