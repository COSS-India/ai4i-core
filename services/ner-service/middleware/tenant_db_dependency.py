"""
Tenant-aware database session dependency for NER Service
"""
...
async def _get_shared_db_session(request: Request) -> AsyncSession:
    ...
    logger.debug("Using shared auth_db session for NER request (no tenant context).")
...
async def get_tenant_db_session(request: Request) -> AsyncSession:
    ...
    TenantMiddleware marks `/api/v1/ner` requests with `needs_tenant_context = True`,
    ...
    if not schema_name and getattr(request.state, "needs_tenant_context", False):
        try:
            ...
            if tenant_context:
                ...
                logger.debug(
                    "Tenant context extracted for NER: tenant_id=%s, schema=%s",
            ...
            # On errors resolving tenant, also fall back to shared auth_db to keep NER functional
            return await _get_shared_db_session(request)
