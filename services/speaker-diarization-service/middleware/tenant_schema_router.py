...
class TenantSchemaRouter:
    ...
    def __init__(self, database_url: str):
        ...
        logger.info(f"TenantSchemaRouter initialized with database URL: {masked_url}")
    ...
    def get_tenant_session_factory(self, schema_name: str) -> async_sessionmaker:
        ...
        if schema_name not in self._session_factories:
            ...
            # Verify which database this engine connects to
            try:
                # Extract database name from URL
                if '/multi_tenant_db' in self.database_url or '/multi-tenant-db' in self.database_url:
                    logger.info(f"Session factory for schema '{schema_name}' connects to multi_tenant_db")
                elif '/auth_db' in self.database_url or '/auth-db' in self.database_url:
                    logger.error(f"WARNING: Session factory for schema '{schema_name}' connects to auth_db instead of multi_tenant_db!")
            except Exception:
                pass
