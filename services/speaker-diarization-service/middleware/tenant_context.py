...
async def try_get_tenant_context(request: Request) -> Optional[Dict[str, Any]]:
    ...
    This is used by services (like Speaker Diarization) that should fall back to shared auth_db
    ...
    logger.info(f"Tenant context resolution - jwt_payload present: {jwt_payload is not None}")
    if jwt_payload:
        # Log JWT payload keys to help debug
        jwt_keys = list(jwt_payload.keys()) if jwt_payload else []
        logger.info(f"JWT payload keys: {jwt_keys}")
        if "tenant_id" in jwt_payload:
            logger.info(f"JWT contains tenant_id: {jwt_payload.get('tenant_id')}")
        if "schema_name" in jwt_payload:
            logger.info(f"JWT contains schema_name: {jwt_payload.get('schema_name')}")
        
        tenant_context = await resolve_tenant_from_jwt(jwt_payload)
        logger.info(f"Tenant context from JWT: {tenant_context}")
        if tenant_context:
            request.state.tenant_context = tenant_context
            request.state.tenant_schema = tenant_context.get("schema_name")
            request.state.tenant_id = tenant_context.get("tenant_id")
            logger.info(f"Resolved tenant context from JWT: tenant_id={tenant_context.get('tenant_id')}, schema={tenant_context.get('schema_name')}")
            return tenant_context
        else:
            logger.warning("JWT payload present but resolve_tenant_from_jwt returned None - tenant_id may be missing from JWT")

    # Fallback: resolve from user_id via API Gateway → multi-tenant-service
    user_id = getattr(request.state, "user_id", None)
    logger.info(f"Tenant context resolution - user_id: {user_id}")
    if not user_id:
        # No authenticated user → no tenant context
        logger.warning("No user_id found in request.state - cannot resolve tenant context")
        return None

    logger.info(f"Attempting to resolve tenant from user_id {user_id} via API Gateway")
    tenant_context = await resolve_tenant_from_user_id(user_id, request)
    logger.info(f"Tenant context from API Gateway: {tenant_context}")
    if not tenant_context:
        # User exists but is not mapped to any ACTIVE tenant (normal user).
        logger.warning(f"User {user_id} is not associated with any active tenant")
        return None

    request.state.tenant_context = tenant_context
    request.state.tenant_schema = tenant_context.get("schema_name")
    request.state.tenant_id = tenant_context.get("tenant_id")
    logger.info(f"Resolved tenant context from API Gateway: tenant_id={tenant_context.get('tenant_id')}, schema={tenant_context.get('schema_name')}")
    return tenant_context
