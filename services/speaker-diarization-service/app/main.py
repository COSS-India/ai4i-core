...
async def lifespan(app: FastAPI):
    ...
    logger.info("Speaker Diarization Service started")
    yield
    ...
    logger.info("Speaker Diarization Service stopped")
...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["speaker-diarization"]
    ...
    if _TELEMETRY:
        try:
            if setup_tracing("speaker-diarization-service"):
    ...
    try:
        ...
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/speaker-diarization"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/speaker-diarization"]
    ...
    async def root():
        return {"service": "speaker-diarization-service", "version": "1.0.0", "status": "running"}
