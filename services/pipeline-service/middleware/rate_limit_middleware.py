    ...
    async def check_rate_limit(self, api_key_id: int, redis_client) -> bool:
        ...
        with tracer.start_as_current_span("redis.rate_limit_check") as span:
            ...
            try:
                ...
                span.set_attribute("redis.minute_count", minute_count)
                span.add_event("redis.operation.complete", {"key": minute_key, "count": minute_count})
                
                if minute_count > self.requests_per_minute:
                    span.set_attribute("rate_limit.result", "rejected")
                    span.set_attribute("rate_limit.limit_type", "minute")
                    span.set_status(Status(StatusCode.OK))
                    return False
                ...
                span.set_attribute("redis.hour_count", hour_count)
                span.add_event("redis.operation.complete", {"key": hour_key, "count": hour_count})
                
                if hour_count > self.requests_per_hour:
                    span.set_attribute("rate_limit.result", "rejected")
                    span.set_attribute("rate_limit.limit_type", "hour")
                    span.set_status(Status(StatusCode.OK))
                    return False
                
                span.set_attribute("rate_limit.result", "allowed")
                span.set_status(Status(StatusCode.OK))
                return True
