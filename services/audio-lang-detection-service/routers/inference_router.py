...
async def _enforce_tenant_and_service_checks(http_request: Request, service_name: str = "audio-lang-detection"):
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if svc_resp.status_code == 200:
                ...
                if not svc_entry or not svc_entry.get("is_active", False):
                    raise HTTPException(status_code=503, detail=ErrorDetail(code="SERVICE_UNAVAILABLE", message="Audio Language Detection service is not active at the moment. Please contact your administrator").dict())
            else:
        ...
        raise HTTPException(status_code=503, detail=ErrorDetail(code="SERVICE_UNAVAILABLE", message="Audio Language Detection service is temporarily unavailable. Please try again in a few minutes.").dict())
    except HTTPException:
        raise
    except Exception as e:
...
inference_router.dependencies.insert(0, Depends(enforce_audio_lang_detection_checks))
