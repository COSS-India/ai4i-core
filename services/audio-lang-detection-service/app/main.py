...
async def lifespan(app: FastAPI):
    ...
    logger.info("Audio Language Detection Service started")
    yield
    ...
    logger.info("Audio Language Detection Service stopped")
...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["audio-lang-detection"]
    ...
    if _TELEMETRY:
        try:
            if setup_tracing("audio-lang-detection-service"):
    ...
    try:
        mm_config = ModelManagementConfig(
            ...
            middleware_paths=["/api/v1/audio-lang-detection"],
            request_timeout=10.0,
        )
        ModelManagementPlugin(config=mm_config).register_plugin(application, redis_client=redis_sync_client)
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/audio-lang-detection"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/audio-lang-detection"]
    ...
    async def root():
        return {"service": "audio-lang-detection-service", "version": "1.0.0", "status": "running"}
