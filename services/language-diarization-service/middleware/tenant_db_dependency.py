...
async def _get_shared_db_session(request: Request):
    """
    ...
    logger.debug("Using shared auth_db session for Language Diarization request (no tenant context).")
...
async def get_tenant_db_session(request: Request):
    ...
    TenantMiddleware marks `/api/v1/language-diarization` requests with `needs_tenant_context = True`,
    ...
    if not schema_name and getattr(request.state, "needs_tenant_context", False):
        try:
            ...
            if tenant_context:
                ...
                logger.debug(
                    "Tenant context extracted for Language Diarization: tenant_id=%s, schema=%s",
            ...
            # On errors resolving tenant, also fall back to shared auth_db to keep Language Diarization functional
            async for session in _get_shared_db_session(request):
                yield session
            return
