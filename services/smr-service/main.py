...
from db_operations import list_all_services, list_services_with_policies
from profiler_routing import call_request_profiler, select_service_by_profiler
from scoring_engine import select_service_deterministically, validate_policy_combinations
...
async def inject_service_id_if_missing(
    ...
    all_headers_provided = (
        latency_policy is not None and cost_policy is not None and accuracy_policy is not None
    )
    ...
    # Priority logic:
    # 1. If ALL three headers are provided -> use them all, skip Policy Engine
    # 2. If SOME headers are provided (1 or 2) -> use headers + get missing ones from tenant policy
    # 3. If NO headers are provided -> use tenant policy for all
    if all_headers_provided:
        # All headers provided - use them directly, skip Policy Engine
        logger.info(
            "SMR: All policy headers provided, using headers directly (skipping Policy Engine)",
            extra={
                "context": {
                    "task_type": task_type,
                    "user_id": user_id,
        ...
        policy_result = None
        logger.info(
            "SMR: All headers provided, skipping Policy Engine call",
            extra={
                "context": {
                    "tenant_id": tenant_id,
                    "routing_decision": "all_headers_priority",
                }
            }
        )
    elif headers_provided:
        # Some headers provided (1 or 2) - use headers + get missing ones from tenant policy
        logger.info(
            "SMR: Partial policy headers provided, using headers + tenant policy for missing ones",
            extra={
                "context": {
                    "task_type": task_type,
                    "user_id": user_id,
                    ...
                    "cost_policy": cost_policy,
                    "accuracy_policy": accuracy_policy,
                    "decision": "partial_headers_with_tenant_policy",
                }
            },
        )
        
        # Call Policy Engine to get tenant policies for missing headers
        if tenant_id is None or tenant_id == "" or tenant_id == "free-user":
        ...
        # Use header if provided, otherwise use tenant policy
        actual_latency_policy = latency_policy if latency_policy is not None else policy_result.get("latency_policy")
        actual_cost_policy = cost_policy if cost_policy is not None else policy_result.get("cost_policy")
        actual_accuracy_policy = accuracy_policy if accuracy_policy is not None else policy_result.get("accuracy_policy")
        ...
        logger.info(
            "SMR: Combined headers with tenant policy",
            extra={
                "context": {
                    "tenant_id": tenant_id,
                    "routing_decision": "partial_headers_with_tenant_policy",
                    "final_latency_policy": actual_latency_policy,
                    "final_cost_policy": actual_cost_policy,
                    "final_accuracy_policy": actual_accuracy_policy,
                    "latency_source": "header" if latency_policy is not None else "tenant_policy",
                    "cost_source": "header" if cost_policy is not None else "tenant_policy",
                    "accuracy_source": "header" if accuracy_policy is not None else "tenant_policy",
                }
            }
        )
    # Priority 3: No headers provided - call Policy Engine for all policies
...
async def select_service(request: Request, payload: SMRSelectRequest) -> SMRSelectResponse:
    ...
    # Validate incompatible combinations of feature headers:
    # - Request profiler or context-aware CANNOT be combined with any of
    #   latency/cost/accuracy policy headers.
    # - Latency/Cost/Accuracy policy headers CAN be combined with each other.
    # - Request profiler and context-aware also MUST NOT be enabled together.
    policy_headers_used = any([
    ...
    # Build list of all used features (for error message)
    if is_request_profiler:
    ...
    # Profiler and context-aware cannot be used together
    if is_request_profiler and is_context_aware:
        incompatible = True

    # Profiler or context-aware cannot be combined with any policy header
    if (is_request_profiler or is_context_aware) and policy_headers_used:
        incompatible = True
    ...
    if incompatible:
        raise HTTPException(
            ...
            detail={
                ...
                "message": (
                    "Incompatible feature headers used together. "
                    "Request profiler and context-aware cannot be combined with "
                    "latency/cost/accuracy policy headers, and cannot be enabled together. "
                    f"You have provided: {', '.join(used_features)}."
                ),
