# POST /api/v1/llm/chat/completion  → messages:[{role,content}]  → choices[0].message.content
# POST /api/v1/llm/generate        → prompt                       → choices[0].text
# OpenAI-shaped JSON; paths are namespaced under /api/v1/llm for the API gateway.
...
router = APIRouter(tags=["chat"], dependencies=[Depends(AuthProvider)])
...
async def chat_completion(
    ...
    return await llm_service.run_chat_completion(
        body,
        request_id,
        user_id=user_id,
