...
async def lifespan(app: FastAPI):
    ...
    app.state.triton_endpoint = app_env.triton_endpoint
    app.state.triton_timeout = app_env.triton_timeout
...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["llm"]
    ...
    try:
        ...
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/llm"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/llm"]
    ...
    async def root():
        return {"service": "llm-service", "version": "1.0.0", "status": "running"}
