...
async def _get_shared_db_session(request: Request) -> AsyncSession:
    ...
    logger.debug("Using shared auth_db session for NMT request (no tenant context).")
    ...
    if not schema_name and getattr(request.state, "needs_tenant_context", False):
        try:
            ...
            if tenant_context:
                ...
                logger.debug(
                    "Tenant context extracted for NMT: tenant_id=%s, schema=%s",
            ...
            # On errors resolving tenant, also fall back to shared auth_db to keep NMT functional
            return await _get_shared_db_session(request)
