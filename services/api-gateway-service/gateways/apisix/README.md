...
- **Production:** Set the three variables in the deployment (e.g. Kubernetes env or ConfigMap) so upstreams are e.g. `simple-ui.sandbox.svc.cluster.local:3000`.

Files:

- **`apisix.yaml`** – Template with placeholders `${APISIX_PUBLIC_HOST}`, `${APISIX_PUBLIC_ORIGIN}`, `${APISIX_UPSTREAM_SUFFIX}`. Mounted as `apisix.yaml.template` in the container; the script writes the resolved **`apisix.yaml`** into the APISIX conf dir before startup.
- **`substitute-env.sh`** – Runs at container start; uses `envsubst` so only these three variables are replaced (other `$` in the YAML, e.g. in regexes, are left unchanged).
