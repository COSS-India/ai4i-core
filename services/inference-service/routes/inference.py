...
async def _run_llm_chat_stream(request: Request, payload: Dict[str, Any], path: str) -> Response:
    """
    Streaming counterpart to _run_llm_chat. MMS resolution, tier gate, and
    model + ai_inference spans are managed inside
    OpenAIProxyService.proxy_traced_stream(), mirroring _run_llm_chat /
    proxy_traced. The upstream connection (and its status code) is resolved
    eagerly so a misconfiguration/upstream failure can still return a normal
    JSON error — only a genuine 200 SSE stream turns into a StreamingResponse.
...
async def _run_llm_chat(request: Request, payload: Dict[str, Any], path: str) -> Response:
    ...
    OpenAIProxyService.proxy_traced() / proxy_traced_stream(), mirroring the
