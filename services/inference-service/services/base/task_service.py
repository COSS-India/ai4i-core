...
class BaseTaskService:
    ...
    def __init__(self, service_info: Optional[Dict[str, Any]] = None):
        ...
        # Cached mapper: pydantic validation in GenericTritonMapper.__init__ costs
        # ~15 ms per chunk call. Cache keyed on adapter_config identity so it
        # rebuilds only when the service descriptor changes (e.g. after MMS refresh).
        self._mapper = None
        self._mapper_cfg = None
    ...
    async def convert_payload_to_triton_format(self, input_data, config):
        ...
        if self._mapper is None or self._mapper_cfg is not cfg:
            self._mapper = GenericTritonMapper(cfg)
            self._mapper_cfg = cfg
        return self._mapper.compose_triton_kserve_v2_payload(
