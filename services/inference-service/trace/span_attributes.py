...
def get_payload_size_kb(data: Any) -> float:
    """
    Calculate JSON serialization size of data in KB.
    
    Returns: size in KB (float), or 0.0 on error
    """
    try:
        if data is None:
            return 0.0
        ...
        logger.warning(f"Error computing payload size: {e}")
        return 0.0
