...
Request payload is analyzed once per dict (``ensure_payload_analyzed``) and cached for
both span attributes and Prometheus emission via observability middleware.
"""
...
def ensure_payload_analyzed(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Analyze the inference payload once and cache for span + Prometheus consumers."""
...
def publish_observability_metrics(
    ...
    """Hand off a single payload analysis snapshot to observability middleware."""
    try:
...
def get_input_type(payload: Dict[str, Any]) -> str:
    ...
    try:
        if isinstance(payload, dict):
            cached = _payload_analysis.get()
            if cached is not None and cached.get("_payload_id") == id(payload):
                return str(cached.get("input_type") or "unknown")
        return _detect_input_type(payload)
...
def count_input_tokens(input_items: List[Any], input_type: str) -> int:
    ...
    try:
        cached = _payload_analysis.get()
        if cached is not None and cached.get("input_type") == input_type:
            return int(cached.get("input_tokens") or 0)
