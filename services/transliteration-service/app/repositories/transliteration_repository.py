"""
Transliteration repository utilities.

Provides CRUD operations for transliteration requests and results.
"""
...
from app.models.transliteration import TransliterationRequestDB, TransliterationResultDB
...
class TransliterationRepository:
    ...
    ) -> Optional[TransliterationRequestDB]:
        """Update transliteration request status and metadata."""
        try:
            result = await self.db.execute(
                select(TransliterationRequestDB).where(TransliterationRequestDB.id == request_id)
            )
            request = result.scalar_one_or_none()

            if not request:
                logger.warning(f"Transliteration request {request_id} not found")
                return None
    ...
    async def get_request_by_id(self, request_id: UUID) -> Optional[TransliterationRequestDB]:
        """Get transliteration request by ID with eager loading of results."""
        try:
            result = await self.db.execute(
    ...
    async def get_requests_by_user(
        ...
        """Get transliteration requests by user ID with pagination."""
        try:
            result = await self.db.execute(
                select(TransliterationRequestDB)
