...
async def lifespan(app: FastAPI):
    ...
    app.state.triton_api_key = getattr(app_env, "triton_api_key", "")
    ...
    logger.info("Transliteration Service started")
    yield
    ...
    logger.info("Transliteration Service stopped")
...
def create_app() -> FastAPI:
    ...
    if _OBSERVABILITY:
        try:
            ...
            config.enabled = True
            config.apps = config.apps or ["transliteration"]
    ...
    if _TELEMETRY:
        try:
            if setup_tracing("transliteration-service"):
    ...
    try:
        ...
        application.add_middleware(AuthContextMiddleware, path_prefixes=["/api/v1/transliteration"])
    except Exception as e:
    ...
    mt_config.tenant_paths = ["/api/v1/transliteration"]
    ...
    async def root():
        return {"service": "transliteration-service", "version": "1.0.0", "status": "running"}
