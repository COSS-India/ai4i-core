...
def add_error_handlers(app: FastAPI) -> None:
    ...
    async def authentication_error_handler(request: Request, exc: AuthenticationError):
        ...
        # PRIORITY 2: Check for expired/invalid token errors FIRST (before ownership check)
        # These should return AUTHENTICATION_ERROR, not AUTHORIZATION_ERROR
        # Check for various token expiration/invalidation messages
        ...
        is_token_error = any(indicator in error_msg.lower() for indicator in token_error_indicators)
        
        # If it's a token error and NOT about API key, return AUTHENTICATION_ERROR
        if is_token_error and "api key" not in error_msg.lower():
            # For expired/invalid tokens, return AUTHENTICATION_ERROR with the original message
            return JSONResponse(
                status_code=401,
                content={
                    "detail": {
        ...
        # PRIORITY 3: For the ownership case, return explicit error + message fields with AUTHORIZATION_ERROR
        ...
        # PRIORITY 4: For invalid API key errors in BOTH mode, check if we have Bearer token
