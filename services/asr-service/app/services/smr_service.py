...
class SMRService:
    """Encapsulates all SMR / fallback-service interactions for ASR."""
    ...
    async def resolve_service_id(
        self,
        request,  # ASRInferenceRequest (Pydantic model)
        ...
        # SMR must be enabled when serviceId is missing
        if not SMR_ENABLED:
            raise HTTPException(
                status_code=400,
                detail={
    ...
    async def call_smr(
        ...
        try:
            ...
            logger.error("SMR service returned error status for ASR", exc_info=True)
            error_detail = None
            try:
    ...
    async def switch_to_fallback(
        ...
        try:
            ...
            logger.error("ASR: Failed to resolve fallback service endpoint", extra={"fallback_service_id": fallback_service_id, "error": str(e)}, exc_info=True)
            raise HTTPException(
                status_code=500,
