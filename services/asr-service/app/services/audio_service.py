...
class AudioService:
    ...
    async def silero_vad_chunking(
        ...
        try:
            ...
            adjusted_timestamps = self.adjust_timestamps(speech_timestamps, sample_rate, max_chunk_duration_s)
            ...
            logger.warning("Falling back to single chunk processing")
            return [audio], [{"start": 0, "end": len(audio), "start_secs": 0, "end_secs": len(audio) / sample_rate}]
    ...
    def adjust_timestamps(
        ...
        for i, timestamp in enumerate(speech_timestamps):
            ...
            if (
                adjusted_timestamps
                and start - adjusted_timestamps[-1]["end"] < 3 * sample_rate
                and end - adjusted_timestamps[-1]["start"] <= max_chunk_duration_samples
            ):
    ...
    async def download_audio(self, url: str) -> bytes:
        ...
        from app.utils.validation_utils import UploadFailedError, UploadTimeoutError
    ...
    def append_silence(self, input_audio: np.ndarray, silence_duration: float, sample_rate: int) -> np.ndarray:
