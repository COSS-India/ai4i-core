...
from app.models.nmt import NMTRequestDB, NMTResultDB
...
class NMTRepository:
    ...
    async def update_request_status(
        ...
        try:
            stmt = (
                ...
                .values(status=status, processing_time=processing_time, error_message=error_message)
    ...
    async def create_results_bulk(
        ...
        try:
            ...
            await self.db.commit()
            logger.info("Created %s NMT results for request %s (bulk)", len(results), request_id)
        except Exception as e:
            await self.db.rollback()
