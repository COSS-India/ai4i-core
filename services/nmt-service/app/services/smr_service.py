"""
SMR (Service Model Router) integration service.
...
class SMRService:
    """Encapsulates all SMR / fallback-service interactions."""
    ...
    async def resolve_service_id(
        self,
        request,  # NMTInferenceRequest (Pydantic model)
        ...
        # ── Context-aware / LLM bypass ──
        context_aware_result = smr_response_data.get("context_aware_result")
        if context_aware_result or service_id == "llm_context_aware":
            logger.info(
                "Context-aware result from SMR, skipping endpoint resolution",
                extra={"service_id": service_id, "has_context_aware_result": bool(context_aware_result)},
            )
    ...
    async def call_smr(
        ...
        try:
            ...
            # Context-aware error passthrough
            headers = dict(http_request.headers)
            ctx_hdr = headers.get("X-Context-Aware") or headers.get("x-context-aware")
            is_ctx = bool(ctx_hdr and ctx_hdr.strip().lower() in {"1", "true", "yes", "y"})
            if is_ctx:
                detail_dict = error_detail if isinstance(error_detail, dict) else {
                    "code": "SMR_SERVICE_ERROR",
                    "message": f"SMR service returned error: {e.response.text}",
                }
                raise HTTPException(status_code=e.response.status_code, detail=detail_dict) from e
    ...
    async def switch_to_fallback(
        ...
        try:
            ...
            logger.error("NMT: Failed to resolve fallback service endpoint", extra={"fallback_service_id": fallback_service_id, "error": str(e)}, exc_info=True)
            raise HTTPException(
                status_code=500,
