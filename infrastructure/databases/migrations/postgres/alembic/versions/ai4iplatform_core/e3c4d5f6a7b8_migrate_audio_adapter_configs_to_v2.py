"""migrate_audio_adapter_configs_to_v2

Revision ID: e3c4d5f6a7b8
Revises: d2b3c4e5f6a7
Create Date: 2026-06-12 00:00:00.000000

Migrates the config-only audio services (Audio Language Detection, Language
Diarization) to the v2 (JSONata) schema (AI4IDS-1981). v1 typed inputs are
preserved; output shaping moves to one output_transform. v1 config is
snapshotted to mm_models_adapter_config_history; downgrade restores it.
Keyed by mm_models.name. Idempotent per row.
"""

import json
from typing import Sequence, Union

from alembic import op
import sqlalchemy as sa


revision: str = "e3c4d5f6a7b8"
down_revision: Union[str, None] = "d2b3c4e5f6a7"
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, None] = None


_ALD_T = (
    '{ "taskType": "audio-lang-detection", "output": [ $map(tensors.LANGUAGE_CODE, '
    'function($lc,$i){ {"language_code": $lc, "confidence": tensors.CONFIDENCE[$i], '
    '"all_scores": tensors.ALL_SCORES[$i]} }) ], '
    '"config": { "serviceId": request.config.serviceId } }'
)
_LD_T = (
    '{ "taskType": "language-diarization", "output": [ tensors.DIARIZATION_RESULT ], '
    '"config": { "serviceId": request.config.serviceId } }'
)

_MIGRATIONS = {
    "ald": (_ALD_T, ["ALL_SCORES"]),
    "lang-diarization": (_LD_T, ["DIARIZATION_RESULT"]),
}


def _load(conn, name):
    row = conn.execute(
        sa.text(
            "SELECT model_id, version, inference_endpoint->'adapter_config' "
            "FROM mm_models WHERE name = :name"
        ),
        {"name": name},
    ).fetchone()
    if row is None or row[2] is None:
        return None
    cfg = row[2] if isinstance(row[2], dict) else json.loads(row[2])
    return row[0], row[1], cfg


def _write(conn, name, cfg):
    res = conn.execute(
        sa.text(
            "UPDATE mm_models SET inference_endpoint = jsonb_set("
            "inference_endpoint, '{adapter_config}', CAST(:cfg AS jsonb)) "
            "WHERE name = :name"
        ),
        {"cfg": json.dumps(cfg), "name": name},
    )
    if res.rowcount != 1:
        raise RuntimeError(f"{name}: expected to update 1 row, got {res.rowcount}")


def upgrade() -> None:
    conn = op.get_bind()
    for name, (transform, json_tensors) in _MIGRATIONS.items():
        loaded = _load(conn, name)
        if loaded is None:
            print(f"  SKIP {name}: no mm_models row / adapter_config")
            continue
        model_id, version, v1 = loaded
        if str(v1.get("schema_version", "")).startswith("2"):
            print(f"  SKIP {name}: already v2")
            continue

        conn.execute(
            sa.text(
                "INSERT INTO mm_models_adapter_config_history "
                "(model_id, version, schema_version, config) "
                "VALUES (:model_id, :version, :schema_version, CAST(:config AS jsonb))"
            ),
            {"model_id": model_id, "version": version,
             "schema_version": str(v1.get("version", "1")), "config": json.dumps(v1)},
        )

        v2 = {
            "schema_version": "2.0",
            "model_version": v1.get("model_version", "1"),
            "inputs": v1.get("inputs", []),
            "outputs": [
                ({"tensor": o["tensor"], "is_json": True}
                 if o["tensor"] in json_tensors else {"tensor": o["tensor"]})
                for o in v1.get("outputs", [])
            ],
            "output_transform": transform,
        }
        _write(conn, name, v2)
        print(f"  OK {name}: adapter_config migrated to v2")


def downgrade() -> None:
    conn = op.get_bind()
    for name in _MIGRATIONS:
        loaded = _load(conn, name)
        if loaded is None:
            continue
        model_id, _version, _cfg = loaded
        row = conn.execute(
            sa.text(
                "SELECT config FROM mm_models_adapter_config_history "
                "WHERE model_id = :model_id ORDER BY archived_at DESC, id DESC LIMIT 1"
            ),
            {"model_id": model_id},
        ).fetchone()
        if row is None:
            raise RuntimeError(f"{name}: no v1 snapshot in history; cannot downgrade")
        v1 = row[0] if isinstance(row[0], dict) else json.loads(row[0])
        _write(conn, name, v1)
        print(f"  OK {name}: adapter_config restored to v1 from history")
