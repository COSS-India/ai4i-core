"""migrate_tts_adapter_config_to_v2

Revision ID: c7d8e9f0a1b2
Revises: b6c7d8e9f0a1
Create Date: 2026-06-12 00:00:00.000000

Migrates TTS to the v2 (JSONata) schema (AI4IDS-1981). TTS is a code-output
service: it keeps its text chunking and waveform DSP and overrides
produce_result to extract OUTPUT_GENERATED_AUDIO from the captured raw Triton
responses, so the v2 config has NO output_transform (only typed inputs + the
output tensor). v1 typed inputs preserved; v1 config snapshotted to
mm_models_adapter_config_history; downgrade restores it. Keyed by
mm_models.name 'tts'. Idempotent.
"""
...
def upgrade() -> None:
    ...
    v2 = {
        ...
        # No output_transform: TTS's produce_result does waveform DSP in code.
    }
