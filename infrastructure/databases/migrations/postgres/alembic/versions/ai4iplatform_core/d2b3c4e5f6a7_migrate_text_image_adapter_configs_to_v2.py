"""migrate_text_image_adapter_configs_to_v2

Revision ID: d2b3c4e5f6a7
Revises: c1a2b3d4e5f6
Create Date: 2026-06-12 00:00:00.000000

Migrates the config-only text/image services (OCR, Transliteration, Language
Detection) to the v2 (JSONata) schema (AI4IDS-1981). Each model's v1 typed
inputs are preserved; its v1 output shaping is replaced by one output_transform
expression. The prior v1 config is snapshotted to mm_models_adapter_config_history
first; downgrade restores it. Keyed by mm_models.name. Idempotent per row.
"""
...
_OCR_T = (
    '{ "output": [ $map(tensors.OUTPUT_TEXT, function($t){ '
    '{"source": $t, "target": ""} }) ], "config": request.config }'
)
_TRANSLIT_T = (
    '( $inp := inputs; { "output": [ $map(tensors.OUTPUT_TEXT, function($t,$i){ '
    '{"source": ($exists($inp[$i].source) ? $inp[$i].source : ""), "target": $t} '
    '}) ] } )'
)
_LANGDET_T = (
    '( $inp := inputs; { "output": [ $map(tensors.OUTPUT_TEXT, function($p,$i){ '
    '{"source": $inp[$i].source, "langPrediction": [$p]} }) ] } )'
)

# model name -> (output_transform, json output tensor names)
_MIGRATIONS = {
    "surya-ocr": (_OCR_T, []),
    "transliteration": (_TRANSLIT_T, []),
    "indiclid": (_LANGDET_T, ["OUTPUT_TEXT"]),
}
...
def upgrade() -> None:
    conn = op.get_bind()
    for name, (transform, json_tensors) in _MIGRATIONS.items():
        loaded = _load(conn, name)
        if loaded is None:
            print(f"  SKIP {name}: no mm_models row / adapter_config")
            continue
        ...
        v2 = {
            ...
            "outputs": [
                ({"tensor": o["tensor"], "is_json": True}
                 if o["tensor"] in json_tensors else {"tensor": o["tensor"]})
                for o in v1.get("outputs", [])
            ],
            "output_transform": transform,
        }
        _write(conn, name, v2)
        print(f"  OK {name}: adapter_config migrated to v2")
...
def downgrade() -> None:
    ...
    for name in _MIGRATIONS:
        ...
        if row is None:
            raise RuntimeError(f"{name}: no v1 snapshot in history; cannot downgrade")
        v1 = row[0] if isinstance(row[0], dict) else json.loads(row[0])
        _write(conn, name, v1)
        print(f"  OK {name}: adapter_config restored to v1 from history")
