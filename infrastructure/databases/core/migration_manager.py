...
class MigrationManager:
    ...
    def __init__(self, migrations_path: str, database_type: str, adapter: BaseAdapter, database_name: str = None):
        """
        ...
        Args:
            ...
            database_name: Specific database name (for postgres: auth_db, config_db, etc.)
        """
    ...
    def make_migration(self, name: str) -> str:
        ...
        # For PostgreSQL, create in database-specific subfolder
        if self.database_type == 'postgres' and self.database_name:
            filepath = self.migrations_path / self.database_type / self.database_name / filename
        else:
    ...
    def _get_all_migration_files(self) -> List[str]:
        ...
        # For PostgreSQL, look in database-specific subfolder
        if self.database_type == 'postgres' and self.database_name:
            db_path = self.migrations_path / self.database_type / self.database_name
        else:
