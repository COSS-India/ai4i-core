...
_API_KEY_PATTERN = re.compile(r"^[0-9a-f]{32}$")
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def authentication_error_handler(
        ...
        # Revoked API key — explicit message from _validate_api_key
        if "APIKey has been revoked" in (error_msg or "") or "has been revoked" in (error_msg or ""):
            return _error_detail_response(401, "API_KEY_REVOKED", "APIKey has been revoked")
        ...
        # ── API key in Authorization header (32-char hex) ──────────────────
        # Detect when the caller used an opaque API key rather than a JWT so
        # we can return a clear, key-specific message instead of the generic
        # "Authentication failed" JWT message.
        auth_header = (
        ...
        if _API_KEY_PATTERN.fullmatch(raw_token.strip()):
            return _error_detail_response(
                401, "INVALID_API_KEY",
                "APIKey has been revoked",
            )
