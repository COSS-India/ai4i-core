...
class PlatformCoreClient:
    ...
    async def list_services(
        ...
        """
        cache_key_suffix = "list_services"
        if task_type:
    ...
    async def get_service(
        ...
        try:
            ...
            if response.status_code == 404:
                logger.warning(
                    f"Service not found (404) for service_id={service_id!r} from platform-core-service at {url}. "
                    "Ensure the service_id exists in the mm_services table (or use a valid service_id)."
                )
                return None
            ...
            if not endpoint or not str(endpoint).strip():
                logger.warning(
                    f"Platform-core-service returned service for service_id={service_id!r} but endpoint is missing or empty. "
