...
Three status.value shapes, one per event category (design doc §6):
  - THRESHOLD (BUDGET_THRESHOLD/QUOTA_THRESHOLD): {"percent": <band>} — the
    highest band recorded as reached. A usage reset (a fresh, lower band)
    differs from whatever was stored too, so it fires again without any
    special-cased reset handling.
  - EXHAUSTED (BUDGET_EXHAUSTED/QUOTA_EXHAUSTED): {"exhausted": true} — an
    on/off flag, fires only on the false/absent -> true transition.
  - Group A admin actions (TIER_ASSIGNED, TIER_CHANGED, BUDGET_ASSIGNED,
    BUDGET_UPDATED, QUOTA_LIMIT_UPDATED): {"occurred_at": <iso ts>} — the
    same occurred_at threaded into the Kafka envelope, so two calls that
...
INSERT ... ON CONFLICT DO UPDATE ... WHERE status IS DISTINCT FROM
EXCLUDED.status RETURNING id. A row coming back means "this is new,
publish"; nothing coming back means "already recorded, skip". This runs as
a real, atomic DB-level guard (not a pure in-memory decision) so two
producer replicas racing the same event can never both "win" — the
in-memory settings cache (notification_id/channels) and the ledger_cache
fast-path below are both pre-checks only, never the source of truth for
dedup.

Before touching the DB at all, ledger_cache.matches_cached_status() is
checked per channel — a HIT (this process already knows the row is at
exactly this status) skips the DB entirely for that channel; a miss/stale/
different-status result falls through to the real UPSERT, which is always
correct regardless of what the cache said. See ledger_cache.py's own
docstring for why this can never cause a missed dedup or a duplicate
publish, only an occasional unnecessary DB call.
...
_UPSERT_SQL = text(
    ...
    DO UPDATE SET
        ...
        updated_at = now()
    WHERE ledger_notification_alert.status IS DISTINCT FROM EXCLUDED.status
    RETURNING id
    """
)
...
async def _record(
    ...
    """True if the ledger did not already reflect `status` for at least one
    configured channel (i.e. this is new — go ahead and publish). False on
    an unknown/not-yet-cached name (fail closed, same reasoning as
    is_notification_enabled) or when every channel already matched."""
...
async def check_and_record_threshold(
    ...
    return await _record(db, name, tenant_id, subject, {"percent": percent}, actor)


async def check_and_record_exhaustion(
    db, name: str, tenant_id: str, subject: Dict[str, Any], actor: str = ""
) -> bool:
    return await _record(db, name, tenant_id, subject, {"exhausted": True}, actor)


async def check_and_record_action(
    db, name: str, tenant_id: str, subject: Dict[str, Any], occurred_at: str, actor: str = ""
) -> bool:
    return await _record(db, name, tenant_id, subject, {"occurred_at": occurred_at}, actor)
