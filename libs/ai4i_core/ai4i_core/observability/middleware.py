...
class ObservabilityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # LLM (chat / chat-completions): input/output tokens and model name
        # are never re-parsed from the response body here — llm_service.py
        # already computed them once (from the same `usage` block, via
        ...
        # route returned) and the SSE stream (multi-chunk — request.state
        # only becomes populated once the stream is fully drained, since
        # Starlette's stream_response() sends the final ASGI message only
        # after the app's own generator, whose post-loop code sets
        # request.state, has completed). Reading request.state in a
        # `finally` after forwarding every chunk untouched is correct for
        # both, and means the body is never buffered — LLM streaming
        # responses reach the client live.
        if service_type == "llm":
    ...
    async def _wrap_llm_response(
        ...
        *,
        ...
        try:
            ...
            service_id = llm_model or (getattr(request.state, "service_id", "") or "")
