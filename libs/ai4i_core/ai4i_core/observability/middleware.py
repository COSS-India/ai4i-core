...
def _application_id_label(request: Request) -> str:
    """Read the X-Application-ID header set by auth-service for API-key calls.

    Returns "" when the header is absent (JWT auth, or pre-cutover series) so
    the metering read-side can detect that case — same reasoning as
    ``_tenant_id_label``.
    """
    return (request.headers.get("X-Application-ID") or "").strip()
...
class ObservabilityMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        application_id_label = _application_id_label(request)
