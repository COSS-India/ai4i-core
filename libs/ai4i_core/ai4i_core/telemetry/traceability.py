...
from ai4i_core.telemetry.propagator import extract_trace_context, get_current_trace_id

from .registry import get_attribute_value
from ai4i_core.context import get_trace_id, set_trace_id
...
def _get_default_mapper_path() -> Path:
    """Resolve default mapper path to libs/ai4i_core/ai4i_core/telemetry/util."""
    current = Path.cwd()

    while current != current.parent:
        lib_telemetry = current / "libs" / "ai4i_core" / "ai4i_core" / "telemetry" / "util"
        if lib_telemetry.exists():
            logger.info(f"[MAPPER PATH] Found at: {lib_telemetry}")
            return lib_telemetry
        current = current.parent

    fallback = Path("libs/ai4i_core/ai4i_core/telemetry/util")
