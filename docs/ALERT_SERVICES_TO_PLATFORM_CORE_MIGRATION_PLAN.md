...
| DB engine / session | Both manage their own `asyncpg.Pool` (`db_pool`, `auth_db_pool`) with manual `init_db_pool`/`close_db_pool`. | `app/core/database.py` → `init_database`/`close_database`/`get_db` (SQLAlchemy async). | **Reuse** SQLAlchemy session for the merged service. Either (a) convert all alert queries to SQLAlchemy Core/ORM, or (b) keep raw SQL but run it through `session.execute(text(...))` so it shares the engine pool. See §7 for the recommended path. The separate `auth_db` connection becomes a second engine (configured via new `AUTH_DB_URL`) — also wired through `app/core/database.py`. |
...
| Versioning / router setup | Alert routers use prefixes like `/alerts/definitions`, no `/api/v1` prefix (gateway rewrites). | platform-core mounts everything under `/api/v1` via `app/routes/__init__.py`. | **Extend** — add the four alert routers to `v1_router` in `app/routes/__init__.py`; **remove** the `proxy-rewrite` strip rule in apisix/nginx. Alert callers will now hit `/api/v1/alerts/...` directly without the `/alerts/...` rewrite. |
...
| Alembic migrations | `infrastructure/databases/migrations/postgres/alembic/migration_registry.py:164` loads `services/alert-management-service/models.py` as the metadata for `alerting_db`. | `migration_registry.py:179` loads `services/platform-core-service/app/models/__init__.py` for `ai4iplatform_core`. | **Extend** `_load_alerting_metadata()` to point at the new location (`services/platform-core-service/app/models/alert/` — see §3). Existing alembic revision (`d7df939ec69b`) does not need to be regenerated — schema stays the same DB (`alerting_db`). |
...
- **Recommended:** alert tables stay in their existing `alerting_db` database. platform-core already targets `ai4iplatform_core` (`CORE_DB_NAME`). Merging schemas into one DB would require schema renames + a data migration — out of scope. So **platform-core needs two engines**: the existing one for `ai4iplatform_core`, plus a new one for `alerting_db`. `auth_db` makes three, but that's already a read-only secondary.
- `app/core/database.py` currently re-exports a single `init_database`/`get_db`. You will need to extend `ai4icore_core.bootstrap.database` (or wrap it locally) to register named engines and produce a `get_alerting_db` / `get_auth_db` dependency. **Confirm with the user**: are they willing to add a multi-engine helper to `ai4icore_core`, or should platform-core wrap it locally?
- **Alternative:** convert alert tables onto SQLAlchemy ORM under the existing `ai4iplatform_core` DB. Requires a data move — flag as risk (§7).
- Existing `alerting_db` Alembic chain (`d7df939ec69b_auto_20260416_155237`) stays intact. Only thing changing is `migration_registry.py:164` `_load_alerting_metadata()` → point at the new `services/platform-core-service/app/models/alert/` package. Until the file move happens you can keep both — i.e. point first at the new path, fall back to old.

### Alembic / migration scripts

- One file to edit: `infrastructure/databases/migrations/postgres/alembic/migration_registry.py:164–169`.
- After the move, `_load_alerting_metadata()` must import the alert ORM from the new platform-core package. Easiest path: dedicated metadata loader that imports `app.models.alert` from `services/platform-core-service/`, identical pattern to `_load_core_service_metadata` at line 179.
- No new revision needed unless you change column types.
...
- Files to update:
  ...
  - Find platform-core's service block and add the env vars + volume mounts that sync needs:
    ...
    - Env: `PROMETHEUS_URL`, `ALERTMANAGER_URL`, `PROMETHEUS_APPLICATION_ALERTS_PATH`, `PROMETHEUS_INFRASTRUCTURE_ALERTS_PATH`, `ALERTMANAGER_CONFIG_PATH`, `SYNC_INTERVAL`, `ALERT_SYNC_ENABLED`, `DEFAULT_RECEIVER_EMAILS`, `AUTH_DB_NAME`, `ALERTING_DB_NAME`.
- API gateway updates:
...
And add three new fields to `CoreSettings`: `alerting_db_name`, `auth_db_name`, plus all the sync-related fields above as `Optional[str]` so platform-core can still boot if alerts feature is disabled.
...
1. **Add `pyyaml>=6.0`** to `services/platform-core-service/requirements.txt`. Add new env keys to `env.template` + `CoreSettings` (all `Optional[...]` so nothing breaks). Add second engine + `get_alerting_db`/`get_auth_db` deps (decide approach with user first — see §7 risk). Repo still builds; alert services untouched.
...
9. **Update Alembic.** Point `migration_registry.py:_load_alerting_metadata()` at the new model location. Run `alembic upgrade head` for `alerting_db` — should be a no-op (same schema).
...
- **Two DBs vs one.** Alert tables live in `alerting_db`; platform-core lives in `ai4iplatform_core`. Plan above keeps two engines. The cleaner long-term design is one DB with `alerting`-prefixed table names — but that's a data migration the user has to schedule. Confirm which path.
...
- **`alerting_db` ORM models use a local `declarative_base()` separate from platform-core's `Base`.** That's actually correct for a multi-engine setup but means `app/models/__init__.py` will export two `Base` objects or you'll keep the alert `Base` private to `app/models/alert/__init__.py`. Confirm convention.
