...
import { performLanguageDiarizationInference } from "../services/languageDiarizationService";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await performLanguageDiarizationInference(
        audioData,
        "ai4bharat/language-diarization"
      );
      ...
      setError(err.message || "Failed to perform language diarization");
      toast({
        title: "Error",
        description: err.message || "Failed to perform language diarization",
        status: "error",
        duration: 5000,
        isClosable: true,
      });
