...
const isAuthenticatedAuthValidationUrl = (rawUrl: string): boolean => {
  const pathNoQuery = (rawUrl.split("?")[0] || "").toLowerCase();
  const suffixes = [apiEndpoints.auth.paths.changePassword].map((s) =>
    s.toLowerCase(),
  );
  return suffixes.some((suffix) => pathNoQuery.endsWith(suffix));
};
...
const getEndpointContext = (rawUrl: string): EndpointContext => {
  ...
  const isPolicyServiceHealthPath = pathNoQuery.endsWith(
    `${apiEndpoints.policy.base}/health`,
  );
...
apiClient.interceptors.request.use(
  async (config: InternalAxiosRequestConfig) => {
    ...
    if (
      (requiresJWT ||
        (context.isAuthEndpoint && !context.isAuthRefreshEndpoint)) &&
      !context.isAuthRefreshEndpoint
    ) {
      try {
        const { default: authService } = await import("./authService");
    ...
    if (
      requiresJWT &&
      (!context.isAuthEndpoint || context.isMultiTenantEndpoint)
    ) {
      ...
      if (jwtToken) {
        ...
        console.warn(
          "No JWT token available for service endpoint:",
          config.url,
        );
    ...
    if (error?.response) {
      ...
      if (responseIndicatesTenantSuspendedOrInactive(status, data)) {
        ...
        if (typeof window !== "undefined" && getJwtToken()) {
          console.warn(
            "API: tenant suspended/deactivated or user inactive — ending session",
          );
          const { forceFrontendSessionEnd } = await import("../hooks/useAuth");
          forceFrontendSessionEnd();
          return Promise.reject(
            new Error(
              "Your organization account is no longer active. Please sign in again.",
            ),
          );
        }
        return Promise.reject(error);
      ...
      switch (status) {
        case 401:
          ...
          if (typeof window !== "undefined") {
            ...
            if (
              context.isServiceEndpoint ||
              context.isModelManagementEndpoint ||
              context.isMultiTenantEndpoint
            ) {
              ...
              const jwtToken = getJwtToken();
              const endpointType = context.isModelManagementEndpoint
                ? "model-management"
                : "service";
              ...
              if (isInvalidAuthCredentials) {
                console.warn(
                  `Invalid authentication credentials for ${endpointType} endpoint - redirecting to sign-in`,
                );
              ...
              if (!originalRequest._retry) {
                ...
                console.warn(
                  "Token refresh already attempted - redirecting to sign-in",
                );
                await clearSessionAndRedirect("/auth");
                return Promise.reject(
                  new Error("Session expired. Please sign in again."),
